...
static TYPE *forward_batch(Transformer *transformer, int *tokens, int *positions, int B) {
    ...
    for (int l = 0; l < p->n_layers; l++) {
        ...
        /* DEBUG: RoPE enabled, multi-head attention bypassed */
        ...
        /* 2f. Multi-head attention: per-token, causal — DEBUG: only Part A enabled */
        for (int b = 0; b < B; b++) {
            int pos = positions[b];
            for (int h = 0; h < p->n_heads; h++) {
                ...
                /* DEBUG: Part B does real reads of att/value_cache but writes constant to xb */
                for (int i = 0; i < head_size; i++) {
