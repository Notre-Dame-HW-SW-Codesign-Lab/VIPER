...
.equ Len_Stack,        0x4000;  // 16kB of stack memory
.equ Len_IRQ_Stack,    0x4000;  // 16kB of stack memory for IRQ Mode
...
fill_cache:
    ...
    /* --- DMA Staging Area Override --- */
    /* Override 0x80C00000 and 0x80D00000 to be Normal Non-Cacheable memory
     * so that the NonCoherentDMA can read CPU writes and CPU can read DMA
     * results without cache coherency issues.
     * ROW=256 staging area spans 2MB (pages 0x80C and 0x80D). */
    ldr r0, =page_table
    ldr r4, =0x80C              @ Index for 0x80C00000
    ldr r2, =0x80C11C02         @ Descriptor: Normal Non-Cacheable, Shareable
    str r2, [r0, r4, lsl #2]    @ Overwrite 1MB entry at 0x80C00000

    ldr r4, =0x80D              @ Index for 0x80D00000
    ldr r2, =0x80D11C02         @ Descriptor: Normal Non-Cacheable, Shareable
    str r2, [r0, r4, lsl #2]    @ Overwrite 1MB entry at 0x80D00000
