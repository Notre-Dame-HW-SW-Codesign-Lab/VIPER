...
main(void)
{
    ...
    stage = 0;
    int done = 0;
    /* Smoke-test heartbeat: print on *every* dispatch so the very first
     * `*top = DEV_INIT` is visible in system.terminal immediately. Cost
     * is one printf per dispatch — negligible vs. the offload. Tighten
     * back to a throttle once the pipeline is proven and we're doing
     * publication-quality runs with millions of dispatches. */
    ...
    for (int i0 = 0; i0 < N; i0 += T) {
        for (int j0 = 0; j0 < N; j0 += T) {
            for (int k0 = 0; k0 < N; k0 += T) {
                /* Stage A sub-tile into m1 (row-major T x T) */
                for (int i = 0; i < T; i++)
                    for (int k = 0; k < T; k++)
                        m1[i * T + k] = A[(i0 + i) * N + (k0 + k)];
                /* Stage B sub-tile into m2 */
                for (int k = 0; k < T; k++)
                    for (int j = 0; j < T; j++)
                        m2[k * T + j] = B[(k0 + k) * N + (j0 + j)];
                ...
                *top   = DEV_INIT;
                ...
                for (int i = 0; i < T; i++)
                    for (int j = 0; j < T; j++)
                        C[(i0 + i) * N + (j0 + j)] += m3[i * T + j];
            }
        }
    }

    /* Row-wise softmax over the full N x N result (numerically stable) */
    for (int i = 0; i < N; i++) {
        TYPE mx = C[i * N];
        for (int j = 1; j < N; j++)
            if (C[i * N + j] > mx) mx = C[i * N + j];
        TYPE sum = (TYPE)0;
        for (int j = 0; j < N; j++) {
            C[i * N + j] = exp_approx(C[i * N + j] - mx);
            sum += C[i * N + j];
        }
        for (int j = 0; j < N; j++)
            C[i * N + j] /= sum;
